...
export function formatTimestamp(iso: string): string {
  ...
  if (diffHours < 24 && d >= today) return `Today at ${timeStr}`;
  if (d >= yesterday && d < today) return `Yesterday at ${timeStr}`;
...
export type MessagePart = { type: 'text'; content: string } | { type: 'bold'; content: string } | { type: 'italic'; content: string } | { type: 'code'; content: string };

/** Parse Discord-style formatting: *bold*, _italic_, `code` into parts for rendering */
export function parseMessageBody(text: string): MessagePart[] {
...
export function shortenPeerId(peerId: string, maxLen = 12): string {
  if (peerId.length <= maxLen) return peerId;
